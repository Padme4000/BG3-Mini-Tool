...
namespace BG3_Mini_Tool
{
    public partial class Form12 : Form
    {
        ...
        {
            InitializeComponent();
            // Populate the dropdown with file types
            comboBoxFileType.Items.Add("All");
            comboBoxFileType.Items.Add("Shorter");
            comboBoxFileType.Items.Add("Default");
            comboBoxFileType.Items.Add("Strong");
        ...
        private string GetSelectedFileType()
        {
            ...
            return comboBoxFileType.SelectedItem.ToString();
        }

        private void UpdateXmlFile(string fileType)
        {
            // Get the path of the selected XML file based on the file type
            string selectedFilePath = GetSelectedFilePath(fileType);

            // Your existing code to perform XML updates goes here, replacing the hardcoded path with selectedFilePath
            // For example:
            // string filePath = selectedFilePath;
            // ...
        }

        private string GetSelectedFilePath(string fileType)
        {
            // Get the directory and base filename
            string directory = Path.Combine(Application.StartupPath, "LSX Files");
            string baseFileName = "CharacterCreationAppearanceVisuals_For";

            // Construct the full file path based on the selected file type
            return Path.Combine(directory, $"{baseFileName}{fileType}.lsx");
        }
        ...
        private void Button6_Click(object sender, EventArgs e)
        {
            ...
            // Get the path of the selected XML file based on the file type
            string filePath = GetSelectedFilePath(selectedFileType);
        ...
        private void Button_add_Click(object sender, EventArgs e)
        {
            ...
            DialogResult result = MessageBox.Show($"Are you sure you want to add the lines from CharacterCreationAppearanceVisuals_{selectedFileType}.lsx to the selected file?", "Confirmation", MessageBoxButtons.YesNo);
            ...
            try
            {
                ...
                string sourceFilePath = GetSelectedFilePath(selectedFileType);
        ...
        private void Button_saveas_Click(object sender, EventArgs e)
        {
            ...
            string originalFilePath = GetSelectedFilePath(selectedFileType);

            // Show the save file dialog
            SaveFileDialog saveFileDialog = new SaveFileDialog();
            saveFileDialog.Filter = "LSX Files (*.lsx)|*.lsx";
            saveFileDialog.FileName = "CharacterCreationAppearanceVisuals.lsx"; // Force the file name
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    // Display a message with the selected version
                    MessageBox.Show($"File saved successfully for version: {selectedFileType}", "File Saved", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
